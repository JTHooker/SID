...
to setup-hex
  clear-links
   ask simuls [
    create-links-with turtles-at 0 1
    create-links-with turtles-at 1 0
    if pxcor mod 2 = 0 [
      create-links-with turtles-at  1 -1
      create-links-with turtles-at -1 -1
      ;; Move every other column of turtles down by half a patch,
      ;; resulting in a hexagonal pattern.
      set ycor ycor - 0.5
    ]
  ]
  reset-ticks
...
;; Events occur. They degrade a number of people's total SIDs to an extent that may or may not mean they lose connection with that SID.
...
;; this is why it is so shocking when people in their 20's or famous people die - all of the net
...
;; n = number of groups
;; i = index of group, starting with 0

;;O = orientation of the group so that Oi = 360 / n

;; Lij is the likelihood of being in group j give you are in group i.
;; Dij is the absolute orientation difference between group i and j
;; Lij = max(0, (1 - 2 * (Dij/360))
;;This formula ensures:

;;    If Dij=0Dij​=0 (same group), Lij=1Lij​=1.
;;    If Dij=90Dij​=90 (adjacent groups), Lij=0.5Lij​=0.5.
;;    If Dij=180Dij​=180 (opposite group), Lij=0Lij​=0.

;; However, if we want a non-zero likelihood for the opposite group, we adjust the formula by adding a base likelihood (BB) for any group, and scale the reduction:
;; Lij = max (B, 1- (Dij/180) * (1 - B))
;; e.g., max (0.1, 1 - (Dij/180) * (1 - B))

;# Base likelihood
;B = 0.01
;
;# Orientation differences
;D_ij_values = [0, 45, 90, 135, 180]
;
;# Calculate likelihoods using the formula
;L_ij_values = [max(B, 1 - D_ij / 180 * (1 - B)) for D_ij in D_ij_values]
;
;L_ij_values
...
to recolororientation
  ask simuls [ ;;fd random-normal 0.01 0.001
  set color  (orientation / 3.6)
  ]
end

to teleport
  ask simuls [
    let colleagues link-neighbors with [abs(heading - [heading] of myself) <= 1]
    if any? colleagues [ move-to one-of colleagues ]]
end

to layout
   layout-spring simuls links Tautness Natural_length Repulsion
end
...
to recolorlinks
   ask simuls [
    set color scale-color red (count my-links) 0 5
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
